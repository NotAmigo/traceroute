...
class Response(object):
    ...
    def __str__(self):
        return f'''
max_ttl = {self.max_rtt}
min_ttl = {self.min_rtt}
avg_ttl = {self.avg_rtt}
    ...
    def do(self):
        ...
        try:  # One could use UDP here, but it's obscure
            if self.udp:
                ...
                current_socket.setsockopt(socket.IPPROTO_IP, socket.IP_TTL, self.ttl)
            else:
...
def ping(hostname, timeout=1000, count=3, packet_size=55, ttl=64, *args, **kwargs):
    p = Ping(hostname, timeout, packet_size, ttl=ttl, *args, **kwargs)
    return p.run(count)
